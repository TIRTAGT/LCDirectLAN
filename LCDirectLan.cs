...
namespace LCDirectLAN
{
	...
	public class LCDirectLan : BaseUnityPlugin
	{
		...
		private void Awake()
		{
			...
			config.Bind<bool>("Latency HUD", "DisableCustomLatencyRPC", false, new ConfigDescription($"Disable {LCDirectLan.PLUGIN_NAME}'s custom RPC and use UnityTransport's GetCurrentRtt() for measuring latency, additional warning feature will be disabled too"));
			config.Bind<bool>("Latency HUD", "HideWhileHosting", true, new ConfigDescription("Hide the Latency HUD when hosting a game, since measuring latency to host (yourself) is not really useful"));
